...
def pickpeak(spec, npicks=2, rdiff=5):
    ...
    for k in range(ncols):  # =1:ncols
        # for a local peak at either end
        dx = np.diff(np.concatenate(
            (rmin.reshape(-1), spec[:], rmin.reshape(-1))))

        # np.where(dx[:mrows] >= 0 and dx[1:] <= 0)[0]
        lp = (dx[:-1] >= 0) & (dx[1:] <= 0)
        ...
        for p in range(npicks):  # 1:npicks
            ...
            ind = np.where(abs(lp[l]-lp) > rdiff)[0]
...
def getF0(y,
    ...
    # Window length large enough to accommodate 3 periods
    win_length = np.ceil(3.0*Fs/min_pitch).astype('int')
    ...
    w = (np.exp(-12.0*(((t+1) * 1.0/win_length)-0.5)**2) -
         np.exp(-12.0))/(1-np.exp(-12.0))

    w = np.concatenate(
        (w, np.zeros(np.ceil(win_length/2.0).astype('int'), dtype='int')))
    ...
    for k in range(kmax):
        ...
        if (max(abs(seg)) < 1e-20):  # % This is a rare case when the signal is a numerical zero throughout
            ...
            # [(k-1)*win_step+1 (k-1)*win_step+win_length];
            T_ind[k, :] = np.linspace(
                (k-1)*win_step, (k-1)*win_step+win_length, win_length, dtype='int')
            continue
        ...
        for ck in range(len(taus)):  # 1:length(taus)  % Interpolate each of the candidates
            ...
            # ranflpd[0.5*l2-1+(startl:T-1)]
            y1 = ranflpd[(0.5*l2-1+(x1)).astype('int')]
        ...
        # ;   % Strength of voiced candidates
        Rvcd = RR[validInd] - octave_cost*(np.log2(min_pitch*TT[validInd]))
        Ro = voicing_thresh + \
            max(0, 2-(lap*(1+voicing_thresh)/(sil_thresh*gap)))
    ...
    # mstar
    # C[kmax].F[mstar[kmax]]
    F0[kmax - 1] = C[kmax - 1]['F'][int(mstar[kmax-1])]
    ...
    for kk in range(1, kmax):  # =kmax-1:-1:1
        ...
        strength[kmax - kk - 1] = C[kmax-kk -
                                    1]['R'][int(mstar[kmax - kk - 1])]
