...
def cropFace(rawImage, landMarks,
    ...
    rightEye = landMarks[rightEyeInds, :].sum(
        0).astype('float')/len(rightEyeInds)
    ...
    for h in range(0, faceHeight):
        ...
        for w in range(0, faceWidth):
            if np.uint16(starty - w * sina) > wi:
                norm_face[h, w] = rawImage[np.uint16(
                    wi), np.uint16(startx + w * cosa)]

            elif np.uint16(startx + w * cosa) > hi:
                norm_face[h, w] = rawImage[np.uint16(
                    starty - w * sina), np.uint16(hi)]

            else:
                norm_face[h, w] = rawImage[np.uint16(
                    starty - w * sina), np.uint16(startx + w * cosa)]
